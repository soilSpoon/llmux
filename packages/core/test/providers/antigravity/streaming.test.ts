...
} from "../../../src/providers/antigravity/streaming";
...
describe("Antigravity Streaming Transformations", () => {
  describe("parseStreamChunk()", () => {
    describe("SSE format parsing", () => {
      ...
      it("should handle whitespace around data", () => {
        const chunk =
          '  data:   {"response":{"candidates":[{"content":{"role":"model","parts":[{"text":"Test"}]}}]}}  ';

        const result = parseStreamChunk(chunk);

        expect(result).not.toBeNull();
        expect((result as StreamChunk | null)?.delta?.text).toBe("Test");
      });
    });
    ...
    describe("text content parsing", () => {
      it("should extract text delta", () => {
        const chunk =
          'data: {"response":{"candidates":[{"content":{"parts":[{"text":"Hello"}],"role":"model"}}]}}';
      ...
      it("should handle empty parts", () => {
        const chunk =
          'data: {"response":{"candidates":[{"content":{"parts":[],"role":"model"}}]}}';

        const result = parseStreamChunk(chunk);

        // May return null or empty content chunk
        expect(result === null || (result as StreamChunk | null)?.delta?.text === undefined).toBe(true);
      });
    });
    ...
    describe("thinking content parsing", () => {
      it("should parse thinking chunk with signature", () => {
        const chunk =
          'data: {"response":{"candidates":[{"content":{"parts":[{"thought":true,"text":"Analyzing...","thoughtSignature":"sig123"}],"role":"model"}}]}}';

        const result = parseStreamChunk(chunk);

        expect((result as StreamChunk | null)?.type).toBe("thinking");
        expect((result as StreamChunk | null)?.delta?.thinking?.text).toBe("Analyzing...");
        expect((result as StreamChunk | null)?.delta?.thinking?.signature).toBe("sig123");
      });

      it("should parse thinking chunk without signature", () => {
        const chunk =
          'data: {"response":{"candidates":[{"content":{"parts":[{"thought":true,"text":"Thinking..."}],"role":"model"}}]}}';
      ...
      it("should parse thinking chunk with snake_case signature", () => {
        const chunk =
          'data: {"response":{"candidates":[{"content":{"parts":[{"thought":true,"text":"Thinking...","thought_signature":"sig_snake"}],"role":"model"}}]}}';
    ...
    describe("tool call parsing", () => {
      it("should parse functionCall chunk", () => {
        const chunk =
          'data: {"response":{"candidates":[{"content":{"parts":[{"functionCall":{"name":"get_weather","args":{"location":"NYC"},"id":"call-123"}}],"role":"model"}}]}}';

        const result = parseStreamChunk(chunk);

        expect((result as StreamChunk | null)?.type).toBe("tool_call");
        expect((result as StreamChunk | null)?.delta?.toolCall?.name).toBe("get_weather");
        expect((result as StreamChunk | null)?.delta?.toolCall?.arguments).toEqual({ location: "NYC" });
        expect((result as StreamChunk | null)?.delta?.toolCall?.id).toBe("call-123");
      });

      it("should handle functionCall without id", () => {
        const chunk =
          'data: {"response":{"candidates":[{"content":{"parts":[{"functionCall":{"name":"search","args":{}}}],"role":"model"}}]}}';

        const result = parseStreamChunk(chunk);

        expect((result as StreamChunk | null)?.type).toBe("tool_call");
        expect((result as StreamChunk | null)?.delta?.toolCall?.name).toBe("search");
        expect((result as StreamChunk | null)?.delta?.toolCall?.id).toBeDefined();
      });
    });
    ...
    describe("finish reason parsing", () => {
      it("should parse STOP finish reason", () => {
        const chunk =
          'data: {"response":{"candidates":[{"content":{"parts":[],"role":"model"},"finishReason":"STOP"}]}}';
      ...
      it("should parse MAX_TOKENS finish reason", () => {
        const chunk =
          'data: {"response":{"candidates":[{"content":{"parts":[],"role":"model"},"finishReason":"MAX_TOKENS"}]}}';

        const result = parseStreamChunk(chunk);

        expect((result as StreamChunk | null)?.stopReason).toBe("max_tokens");
      });

      it("should parse SAFETY finish reason", () => {
        const chunk =
          'data: {"response":{"candidates":[{"content":{"parts":[],"role":"model"},"finishReason":"SAFETY"}]}}';

        const result = parseStreamChunk(chunk);

        expect((result as StreamChunk | null)?.stopReason).toBe("content_filter");
      });
    });
    ...
    describe("usage metadata parsing", () => {
      it("should parse usage in final chunk", () => {
        const chunk =
          'data: {"response":{"candidates":[{"content":{"parts":[],"role":"model"},"finishReason":"STOP"}],"usageMetadata":{"promptTokenCount":100,"candidatesTokenCount":200,"totalTokenCount":300}}}';

        const result = parseStreamChunk(chunk);

        expect((result as StreamChunk | null)?.usage).toBeDefined();
        expect((result as StreamChunk | null)?.usage?.inputTokens).toBe(100);
        expect((result as StreamChunk | null)?.usage?.outputTokens).toBe(200);
        expect((result as StreamChunk | null)?.usage?.totalTokens).toBe(300);
      });

      it("should parse thinking tokens in usage", () => {
        const chunk =
          'data: {"response":{"candidates":[{"finishReason":"STOP","content":{"parts":[],"role":"model"}}],"usageMetadata":{"promptTokenCount":100,"candidatesTokenCount":200,"totalTokenCount":300,"thoughtsTokenCount":50}}}';

        const result = parseStreamChunk(chunk);

        expect((result as StreamChunk | null)?.usage?.thinkingTokens).toBe(50);
      });

      it("should parse cached tokens in usage", () => {
        const chunk =
          'data: {"response":{"candidates":[{"finishReason":"STOP","content":{"parts":[],"role":"model"}}],"usageMetadata":{"promptTokenCount":100,"candidatesTokenCount":200,"totalTokenCount":300,"cachedContentTokenCount":25}}}';

        const result = parseStreamChunk(chunk);

        expect((result as StreamChunk | null)?.usage?.cachedTokens).toBe(25);
      });
    });
    ...
    describe("error handling", () => {
      ...
      it("should handle missing response wrapper", () => {
        ...
        expect(result === null || (result as StreamChunk | null)?.delta?.text === "Hi").toBe(true);
      });
  ...
  describe("round-trip", () => {
    ...
    it("should preserve thinking content through round-trip", () => {
      ...
      expect((parsed as StreamChunk)?.delta?.thinking?.text).toBe(
        "Deep thought..."
      );
      expect((parsed as StreamChunk)?.delta?.thinking?.signature).toBe(
        "sigABC"
      );
    });
