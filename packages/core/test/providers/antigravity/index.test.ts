...
import type { UnifiedRequest, UnifiedResponse, StreamChunk } from '../../../src/types/unified'
import type { AntigravityRequest, AntigravityResponse } from '../../../src/providers/antigravity/types'
import { createUnifiedRequest, createUnifiedMessage, createUnifiedResponse, createUnifiedTool } from '../_utils/fixtures'
import { expectRequestRoundTrip, expectResponseRoundTrip, collectStreamChunks } from '../_utils/helpers'
...
describe('AntigravityProvider', () => {
  ...
  describe('transformResponse()', () => {
    it('should transform a simple UnifiedResponse', () => {
      ...
      const result = provider.transformResponse(response) as AntigravityResponse

      expect(result.response.candidates[0].content.parts[0].text).toBe('Hello!')
  ...
  describe('parseStreamChunk()', () => {
    it('should parse text stream chunk', () => {
      const chunk = 'data: {"response":{"candidates":[{"content":{"role":"model","parts":[{"text":"Hi"}]}}]}}'
    ...
    it('should parse thinking stream chunk', () => {
      const chunk = 'data: {"response":{"candidates":[{"content":{"role":"model","parts":[{"thought":true,"text":"Thinking...","thoughtSignature":"sig"}]}}]}}'
    ...
    it('should parse done chunk', () => {
      const chunk = 'data: {"response":{"candidates":[{"content":{"role":"model","parts":[]},"finishReason":"STOP"}]}}'
