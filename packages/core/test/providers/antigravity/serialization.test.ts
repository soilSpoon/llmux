...
describe('Antigravity Serialization', () => {
  ...
  it('should integration test via AntigravityProvider transform', () => {
    ...
    if (genConfig) {
      ...
      // The AntigravityProvider.transform returns AntigravityInnerRequest which has camelCase keys
      // It is only converted to snake_case when calling convertToWireFormat
      expect(genConfig.thinkingConfig).toBeDefined()
