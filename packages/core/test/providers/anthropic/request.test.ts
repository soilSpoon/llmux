...
import type { UnifiedRequest, UnifiedMessage, ContentPart } from '../../../src/types/unified'
...
describe('Anthropic Request Transformations', () => {
  describe('transform (UnifiedRequest → AnthropicRequest)', () => {
    ...
    it('should transform image URL content parts', () => {
      ...
      expect((content[0] as any).source.url).toBe('https://example.com/image.jpg')
    })
    ...
    it('should transform thinking parts in assistant messages', () => {
      ...
      expect((content[0] as any).thinking).toBe('Let me analyze this step by step...')
      expect((content[0] as any).signature).toBe('EqQBCgIYAhIM1gbcDa9GJwZA2b3hGgxBdjrkzLoky3dl1pk')
    })
  ...
  describe('parse (AnthropicRequest → UnifiedRequest)', () => {
    ...
    it('should parse image URL source', () => {
      ...
      expect(result.messages[0].parts[0].image?.url).toBe('https://example.com/img.jpg')
    })
    ...
    it('should parse tool_use blocks to tool_call parts', () => {
      ...
      expect(result.messages[0].parts[1].toolCall?.arguments).toEqual({ location: 'NYC' })
    })
    ...
    it('should parse tool_result blocks to tool_result parts', () => {
      ...
      expect(result.messages[0].parts[0].toolResult?.toolCallId).toBe('toolu_123')
    ...
    it('should parse thinking blocks', () => {
      ...
      expect(result.messages[0].parts[0].thinking?.text).toBe('Let me analyze...')
