...
describe('Anthropic Types', () => {
  ...
  describe('AnthropicMessage', () => {
    ...
    it('should support assistant message with tool_use', () => {
      ...
      const toolUse = (msg.content as AnthropicContentBlock[]).find((b) => b.type === 'tool_use')
    ...
    it('should support user message with tool_result', () => {
      const msg: AnthropicMessage = {
        role: 'user',
        content: [{ type: 'tool_result', tool_use_id: 'toolu_123', content: '{"temp": 72}' }],
      }
      const toolResult = (msg.content as AnthropicContentBlock[])[0]
  ...
  describe('Type Guards', () => {
    ...
    describe('isAnthropicMessage', () => {
      it('should return true for valid message', () => {
        expect(isAnthropicMessage({ role: 'user', content: 'Hello' })).toBe(true)
        expect(isAnthropicMessage({ role: 'assistant', content: [] })).toBe(true)
      })

      it('should return false for invalid role', () => {
        expect(isAnthropicMessage({ role: 'system', content: 'Hello' })).toBe(false)
      })
    })
    ...
    describe('isAnthropicStreamEvent', () => {
      it('should return true for valid stream events', () => {
        ...
        expect(isAnthropicStreamEvent({ type: 'content_block_delta', index: 0, delta: {} })).toBe(
          true
        )
