...
import { BaseProvider, type ProviderConfig, type ProviderName } from '../../src/providers/base'
...
describe('Provider Registry', () => {
  ...
  describe('getProvider', () => {
    ...
    it('should throw error for unregistered provider', () => {
      expect(() => getProvider('openai')).toThrow('Provider "openai" not registered')
    })
    ...
    it('should work with minimal provider implementation', () => {
      class MinimalProvider extends BaseProvider {
        ...
        parse(): UnifiedRequest {
          return { messages: [{ role: 'user', parts: [{ type: 'text', text: 'test' }] }] }
        }
        transform(): unknown {
          return { contents: [] }
        }
        parseResponse(): UnifiedResponse {
          return { id: 'test', content: [{ type: 'text', text: 'response' }], stopReason: 'end_turn' }
        }
