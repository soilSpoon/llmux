...
import { parseResponse, transformResponse } from '../../../src/providers/ai-sdk/response'
...
describe('AI SDK Response Transformations', () => {
  describe('parseResponse', () => {
    ...
    it('parses various finish reasons', () => {
      const testCases: Array<{ unified: 'stop' | 'length' | 'tool-calls' | 'content-filter' | 'error' | 'other', expected: string | null }> = [
  ...
  describe('transformResponse', () => {
    ...
    it('transforms response with thinking blocks', () => {
      ...
      expect((result.content[0] as { text: string }).text).toBe('Let me think...')
    ...
    it('transforms response with tool call', () => {
      ...
      const toolCall = result.content[0] as { toolCallId: string; toolName: string; input: string }
