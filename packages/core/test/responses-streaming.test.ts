...
describe("Responses API Streaming", () => {
  ...
  describe("ResponsesStreamTransformer", () => {
    ...
    it("should generate unique response and message IDs", () => {
      ...
      const outputItemEvent = events.find(
        (e) => e.type === "response.output_item.added"
      ) as ResponsesOutputItemAddedEvent | undefined;
      ...
      ) as ResponsesOutputTextDeltaEvent | undefined;
    ...
    it("should accumulate text across multiple chunks", () => {
      ...
      const deltaEvent1 = events1.find(
        (e) => e.type === "response.output_text.delta"
      );
      const deltaEvent2 = events2.find(
        (e) => e.type === "response.output_text.delta"
      );
      const completedEvent = events3.find(
  ...
  describe("Full Streaming Round-trip", () => {
    it("should transform a complete streaming conversation", () => {
      ...
      const completedEvent = allEvents.find(
        (e) => e.type === "response.completed"
      );
      expect(completedEvent).toBeDefined();
      if (completedEvent && completedEvent.type === "response.completed") {
      ...
      const contentPartIndex = eventTypes.indexOf(
  ...
  describe("Error Handling", () => {
    ...
    it("should handle rapid sequential chunks", () => {
      const transformer = new ResponsesStreamTransformer("gpt-4o");
      const chunks: ChatCompletionChunk[] = Array.from(
        { length: 100 },
        (_, i) => ({
          id: "chatcmpl-1",
          object: "chat.completion.chunk",
