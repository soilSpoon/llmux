...
describe("sanitizeSchema", () => {
  ...
  describe("nested sanitization", () => {
    it("should sanitize nested properties", () => {
      ...
      const nameProp = result.properties?.user?.properties?.name as Record<
        string,
        unknown
      >;
    ...
    it("should sanitize items in array schemas", () => {
      ...
      expect(
        (result.items?.properties?.id as Record<string, unknown>).examples
      ).toBeUndefined();
    });
    ...
    it("should sanitize anyOf/oneOf/allOf schemas", () => {
      ...
      expect(
        (result.anyOf?.[0] as Record<string, unknown>).title
      ).toBeUndefined();
      expect(
        (result.anyOf?.[0] as Record<string, unknown>).default
      ).toBeUndefined();
      expect(
        (result.anyOf?.[1] as Record<string, unknown>).examples
      ).toBeUndefined();
    });
    ...
    it("should sanitize additionalProperties when it is a schema", () => {
      ...
      expect((result.additionalProperties as JSONSchemaProperty).type).toBe(
        "string"
      );
    });
  });
  ...
  describe("edge cases", () => {
    ...
    it("should handle deeply nested schemas", () => {
      ...
      const result = sanitizeSchema(input);
      const level2 = result.properties?.level1?.properties?.level2 as Record<
        string,
        unknown
      >;
      const level3 = result.properties?.level1?.properties?.level2?.properties
