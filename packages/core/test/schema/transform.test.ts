...
describe("constToEnum", () => {
  it("should convert const to enum with single value", () => {
    const input = { const: "fixed_value" } as JSONSchemaProperty & {
      const?: unknown;
    };
  ...
  it("should convert const in nested properties", () => {
    ...
    expect(
      (result.properties?.status as Record<string, unknown>).const
    ).toBeUndefined();
  });
});
...
describe("inlineRefs", () => {
  it("should inline $ref from $defs", () => {
    ...
    expect(
      (result.properties?.home as Record<string, unknown>).$ref
    ).toBeUndefined();
...
describe("anyOfToSnakeCase", () => {
  ...
  it("should convert nested anyOf", () => {
    ...
    expect(
      (result.properties?.value as JSONSchemaProperty).anyOf
    ).toBeUndefined();
  });
...
describe("transformSchema", () => {
  it("should apply all transformations", () => {
    ...
    expect(result.properties?.status?.enum).toEqual(["active"]);
    expect(
      (result.properties?.value as Record<string, unknown>).any_of
    ).toBeDefined();
  ...
  it("should handle complex nested transformations", () => {
    ...
    expect(
      (result.items?.properties?.data as Record<string, unknown>).any_of
    ).toBeDefined();
  });
});
