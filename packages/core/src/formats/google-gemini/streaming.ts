/**
 * Google Gemini Streaming Transformations
...
function parseSingleChunk(jsonStr: string): StreamChunk | StreamChunk[] | null {
  ...
  try {
    ...
    // Handle Antigravity/Vertex wrapped format
    if (parsed && typeof parsed === 'object' && 'response' in parsed) {
      data = parsed.response as GeminiStreamChunk
    } else {
      data = parsed as GeminiStreamChunk
    }
  } catch {
    return null
  }
 ...
 */
export function buildStreamChunk(chunk: StreamChunk): string {
