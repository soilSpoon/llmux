...
export class AntigravityStreamingBuilder {
  ...
  constructor(state: AntigravityBuilderState, model: string) {
    ...
    // Initialize normalization state if missing (for existing sessions/tests)
    if (!this.state.normalization) {
  ...
  build(chunk: StreamChunk | StreamChunk[]): string | string[] | null {
    ...
    for (const c of chunks) {
      // Normalize chunks
      const normalizedChunks = normalizeStreamingOrder(c, this.state.normalization)
  ...
  private processChunk(c: StreamChunk): string[] {
    const results: string[] = []
