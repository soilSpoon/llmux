...
import type { AuthProvider } from "../../src/providers/base";
...
describe("AuthProvider interface", () => {
  ...
  test("getHeaders returns header object", async () => {
    const headers = await mockProvider.getHeaders({ type: "api", key: "test" });
  ...
  test("method authorize returns AuthResult", async () => {
    ...
    expect((result as any).credential).toBeDefined();
  });
});
