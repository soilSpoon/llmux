...
describe("GithubCopilotProvider", () => {
  ...
  test("supports device-flow method", () => {
    const deviceMethod = GithubCopilotProvider.methods.find(
      (m) => m.type === "device-flow"
    );
...
describe("GithubCopilotDeviceFlow", () => {
  ...
  test("requestDeviceCode returns code info", async () => {
    ...
    global.fetch = mockFetch(
      async () => new Response(JSON.stringify(mockResponse))
    );
...
describe("pollForToken", () => {
  ...
  test("returns success with valid token", async () => {
    ...
    if (
      result.type === "success" &&
      result.credential &&
      isOAuthCredential(result.credential)
    ) {
  ...
  test("returns success with default expiresIn when not provided", async () => {
    ...
    if (
      ...
      expect(result.credential.expiresAt).toBeGreaterThanOrEqual(
        beforeTime + 28800 * 1000
      );
    }
  });
