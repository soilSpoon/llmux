...
// Result type interfaces for type-safe assertions
interface IntermediateAuthStep {
  type: "intermediate";
  url: string;
  message: string;
  auto: boolean;
  callback: (input?: string) => Promise<SuccessAuthResult | FailedAuthResult>;
}
...
function isIntermediateAuthStep(
  result: unknown
): result is IntermediateAuthStep {
  return (
    typeof result === "object" &&
    result !== null &&
    (result as IntermediateAuthStep).type === "intermediate"
  );
}
...
describe("refreshAntigravityToken", () => {
  ...
  test("refreshes token successfully", async () => {
    ...
    const { refreshAntigravityToken } = await import(
  ...
  test("throws error when refresh token is undefined", async () => {
    ...
    const currentCredential = castTo<OAuthCredential>({
      type: "oauth",
      accessToken: "some_token",
...
describe("authorizeAntigravity", () => {
    ...
    mock.module("../src/providers/antigravity-server", () => ({
      startOAuthListener: async () => ({
        waitForCallback: async () =>
          new URL("http://localhost:51121/oauth-callback"),
        close: async () => {},
      }),
    }));

    const { authorizeAntigravity } = await import(
      "../src/providers/antigravity-oauth"
    );
    const result = (await authorizeAntigravity()) as IntermediateAuthStep;
  ...
  test("callback returns failed when no code in callback URL", async () => {
    ...
    expect((authResult as FailedAuthResult).error).toBe(
      "No code received in callback"
    );
  });
  ...
  test("callback returns failed on token exchange failure", async () => {
    mock.module("../src/providers/antigravity-server", () => ({
      startOAuthListener: async () => ({
        waitForCallback: async () =>
          new URL("http://localhost:51121/oauth-callback?code=valid_code"),
        close: async () => {},
      }),
    }));
    ...
    expect(authResult.type).toBe("failed");
    expect((authResult as FailedAuthResult).error).toContain(
  ...
  test("callback returns success with valid OAuth flow", async () => {
    ...
    const credential = (authResult as SuccessAuthResult).credential;
  ...
  test("callback handles errors and closes listener", async () => {
    ...
    expect(authResult.type).toBe("failed");
    expect((authResult as FailedAuthResult).error).toBe("Connection timeout");
  ...
  test("callback handles non-Error exceptions", async () => {
    ...
    expect(authResult.type).toBe("failed");
    expect((authResult as FailedAuthResult).error).toBe("Unknown error");
  });
  ...
  test("closes listener after successful token exchange", async () => {
    ...
    mock.module("../src/providers/antigravity-server", () => ({
      startOAuthListener: async () => ({
        waitForCallback: async () =>
          new URL("http://localhost:51121/oauth-callback?code=valid"),
        close: async () => {
          listenerClosed = true;
        },
      }),
    ...
    global.fetch = createMockFetch(async (url: string | URL | Request) => {
      ...
      if (urlStr.includes("loadCodeAssist")) {
        return new Response(
          JSON.stringify({ cloudaicompanionProject: "proj" }),
          { status: 200 }
        );
      }
...
describe("fetchProjectID integration tests", () => {
  ...
  test("returns project ID from string response", async () => {
    mock.module("../src/providers/antigravity-server", () => ({
      startOAuthListener: async () => ({
        waitForCallback: async () =>
          new URL("http://localhost:51121/oauth-callback?code=test"),
        close: async () => {},
      }),
    }));
  ...
  test("handles empty string cloudaicompanionProject", async () => {
    ...
    global.fetch = createMockFetch(async (url: string | URL | Request) => {
      ...
      if (urlStr.includes("loadCodeAssist")) {
        return new Response(JSON.stringify({ cloudaicompanionProject: "" }), {
          status: 200,
        });
      }
      return new Response("Not Found", { status: 404 });
    ...
    const result = (await authorizeAntigravity()) as IntermediateAuthStep;
    expect(result.type).toBe("intermediate");
    const authResult = await (result as IntermediateAuthStep).callback!();
