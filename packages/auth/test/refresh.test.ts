  ...
  describe("OAuth refresh flow", () => {
    test("calls provider refresh when token needs refresh", async () => {
      ...
      const mockRefresh = mock(
        async (_cred: OAuthCredential) => refreshedToken
      );
    ...
    test("refreshes multiple expiring credentials", async () => {
      ...
      let callCount = 0;
      const mockRefresh = mock(
        async (cred: OAuthCredential): Promise<OAuthCredential> => {
          callCount++;
          return {
            ...cred,
      ...
      expect(result).toHaveLength(2);
      expect((result[0] as OAuthCredential)?.accessToken).toBe("refreshed-1");
      expect((result[1] as OAuthCredential)?.accessToken).toBe("refreshed-2");
