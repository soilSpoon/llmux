...
describe("ConfigLoader", () => {
  ...
  test("getPath returns correct path", () => {
    ...
    expect(path).toBe(join(tempDir, ".llmux", "config.json"));
  });
  ...
  test("load returns default config when no file exists", async () => {
    ...
    expect(config.routing.fallbackOrder).toContain("anthropic");
  });
  ...
  test("save and load config", async () => {
    ...
    expect(loaded.routing.fallbackOrder?.[0]).toBe("openai");
