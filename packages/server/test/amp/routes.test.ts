...
import {
  createAmpRoutes,
  type AmpRoutesConfig,
  type ProviderHandlers,
} from '../../src/amp/routes'
import { FallbackHandler, type ProviderChecker } from '../../src/handlers/fallback'
...
describe('Provider routing', () => {
  const mockProviderResponses: Record<string, string> = {
...
describe('Models endpoint', () => {
  test('should return provider-specific models list', async () => {
    const modelsHandler = async (_req: Request, params?: Record<string, string>) => {
    ...
    const request = new Request('http://localhost/api/provider/openai/v1/models', {
      method: 'GET',
    })
...
describe('FallbackHandler integration', () => {
  ...
  test('should apply fallback handler to POST endpoints', async () => {
    ...
    const fallbackHandler = new FallbackHandler(() => mockProxy, providerChecker)
