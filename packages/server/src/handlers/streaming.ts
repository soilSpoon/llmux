...
export function transformStreamChunk(
  ...
  try {
    ...
    if (
      !sourceProvider.parseStreamChunk ||
      !targetProvider.transformStreamChunk
    ) {
      return chunk;
    }
...
export async function handleStreamingProxy(
  ...
  try {
    ...
    if (options.targetProvider === "opencode-zen" && originalModel) {
      if (
        originalModel === "glm-4.7-free" ||
        originalModel.includes("claude")
      ) {
    ...
    if (originalModel) {
      const appliedMapping = applyModelMapping(
        originalModel,
        ...
        logger.info(
          {
            ...
            mappings:
              options.modelMappings?.map(
                (m) =>
                  `${m.from}->${Array.isArray(m.to) ? m.to.join(",") : m.to}`
              ) || [],
          },
      ...
      if (
        options.targetProvider !== "antigravity" ||
        appliedMapping !== originalModel
      ) {
    ...
    if (options.targetProvider === "antigravity" && mappedModel) {
      ...
      if (aliasedModel !== mappedModel) {
        logger.info(
          { originalModel: mappedModel, aliasedModel },
          "Antigravity model alias applied"
        );
    ...
    if (authProvider && (!options.apiKey || options.apiKey === "dummy")) {
      ...
      let credentials:
        | Awaited<ReturnType<typeof TokenRefresh.ensureFresh>>
        | undefined;
      try {
      ...
      if (
        options.targetProvider === "opencode-zen" &&
        effectiveTargetProvider === "anthropic"
      ) {
        headers["anthropic-version"] = "2023-06-01";
    ...
    const transformStream = new TransformStream<Uint8Array, Uint8Array>({
      transform(chunk, controller) {
        ...
        if (parsingProvider === "antigravity" || parsingProvider === "gemini") {
          ...
          const linesToProcess = lastLineIncomplete
            ? lines.slice(0, -1)
            : lines;
          const remainingLine = lastLineIncomplete
          ...
          if (parsingProvider !== sourceFormat) {
            try {
              ...
              logger.debug(
                {
                  ...
                  to: sourceFormat,
                  transformedType: Array.isArray(transformed)
                    ? "array"
                    : "string",
              ...
              const detectBlockType = (
                sse: string
              ): "thinking" | "text" | "tool_use" | "stop" | null => {
                if (
                  sse.includes('"type":"message_stop"') ||
                  sse.includes('"type":"message_delta"')
                ) {
                  return "stop";
                }
              ...
              const sendBlockStart = (
                ...
                if (blockType === "thinking") {
                  ...
                  logger.debug(
                    { index, blockType },
                    "[streaming] Sending block start (thinking)"
                  );
                  ...
                  logger.debug(
                    { index, blockType },
                    "[streaming] Sending block start (text)"
                  );
                  controller.enqueue(encoder.encode(event));
                }
              };
              ...
              const processChunk = (chunk: string) => {
                ...
                const isBlockStart = chunk.includes(
                  '"type":"content_block_start"'
                );
                const isBlockStop = chunk.includes(
                ...
                if (sourceFormat === "anthropic") {
                  ...
                  if (isBlockStart) {
                    ...
                    chunkBlockType &&
                    chunkBlockType !== currentBlockType
                  ) {
                ...
                if (
                  ...
                  const thinkingMatch =
                    chunk.match(/"thinking":"([^"]*)"/)?.[1];
                  const signatureMatch = chunk.match(
                    /"signature":"([^"]*)"/
                  )?.[1];
